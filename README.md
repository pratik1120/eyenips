...
**✎ Edit equations — the generator is open, not a black box.** Click **✎ Edit
equations…** (next to Randomize) and you get the actual archetype **formulas** and
the **recipe ranges** as editable text:

- Each archetype is a live-compiled formula in `u, v` (position), `t` (time),
  `f1/f2/f3` (recipe frequencies) and `ph` (phase) — e.g. `sin(u*f1+t)+sin(v*f2-t)`
  — with `sin cos sqrt abs floor min max atan2 round …` and the building blocks
  `vor()`, `julia()`, `fbm()`, `vnoise()`. **Edit** them, **+ Add archetype** of
  your own, or remove ones you don't like. Bad math shows an error and never
  crashes (it keeps the last working version).
- The **recipe ranges** (frequency spans, symmetry/colour/style odds, warp
  amounts) decide how a Recipe number rolls into an effect — all editable.
- **Reset to defaults** restores the original equations and ranges. Your edits are
  saved to `~/.eyenips/lab_kit.json` and persist across sessions.

Same compiler the Create-Effect editor uses — no generative AI, just your math.
...
**✎ Edit equations** here too: choose **which operators** are in the random pool
(uncheck any you never want) and tune the **recipe ranges** — operator sparsity
(how many dominant vs accent), frequency spans, mosaic/kaleidoscope amounts, and
more. **Reset to defaults** restores the original set; edits persist in
`~/.eyenips/lab_kit.json`.
